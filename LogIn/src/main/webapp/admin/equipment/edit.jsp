        ...
        <form action="/amv/admin/equipment?edit=<%=getEquipment(request) != null ? getEquipment(request).getId() : ""%>"
            ...
            <label>
                ...
                <input type="number" name="price" step="1"
                       value="<%=getEquipment(request) != null ? getEquipment(request).getPrice() / 100 : ""%>"
                       required>
            </label>
            <label>
                Rent start on day:
                <input type="number" name="rent-starts-on-day"
                       value="<%=getEquipment(request) != null ? getEquipment(request).getRentStartDay() : ""%>"
                       required>
            </label>
            <label>
                max-rental-days:
                <input type="number" name="max-rental-days"
                       value="<%=getEquipment(request) != null ? getEquipment(request).getMaxRentalDays() : ""%>"
                       required>
            </label>
            <label>
                Comment:
